<style lang="less">
	page{
		padding-bottom: 122rpx!important;
	}
</style>
<template>
          ...
          <block v-for="(item, vipListIndex) in vipList" :key="vipListIndex">
            <swiper-item>
              <view class="swiper-slide" :style="{ backgroundImage: 'url(' + item.image + ')' }">
                ...
                <view class="nav acea-row" style="width: 70%;" v-if="item.grade == grade">
                  <view class="item" v-for="(val, indexn) in vipComplete" :key="indexn">
                    <view style="font-size: 30rpx;" class="num">{{ val.newNumber }}</view>
                    <view style="font-size: 30rpx;">{{ val.realName }}</view>
                  </view>
                </view>
